...
for(var i = 0; i < selColorBtns.length; i++){
  selColorBtns[i].addEventListener('click', function(){
    ...
    var color_id = this.dataset.value
    console.log(productId,action,color_id,size_id)
...
for(var i = 0; i < selSizeBtns.length; i++){
  selSizeBtns[i].addEventListener('click', function(){
    var productId = this.dataset.product
    var action = this.dataset.action
    var size_id = this.dataset.value
    console.log(productId,action,size_id,color_id)
    // if(user == 'AnonymousUser'){
    //   addCookieItem(productId, action)
...
//   var url = '/update_variation/'
  ...
  //   },
  //   body:JSON.stringify({'productId': productId, 'action': action, 'var_id': var_id})
  // })
